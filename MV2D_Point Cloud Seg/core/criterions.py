...
class WeaklyLcLoss(nn.Module):
    def __init__(self, weight=None, ignore_index=255):
        super(WeaklyLcLoss, self).__init__()
        self.ignore_index = ignore_index
        if weight is None:
            self.ce = nn.CrossEntropyLoss(ignore_index=ignore_index)
        else:
    ...
    def forward(self, out_dict: dict, y):
        x_vox = out_dict.get('x_vox')
        x_pix = out_dict.get('x_pix')
        l1 = self.ce(x_vox, y)  # 使用点云特征的预测结果
        l2 = self.ce(x_pix, y)  # 使用图像特征的预测结果
        return {
            'predict_vox': l1,
            'predict_pix': l2
        }
...
class MixVCDCrossEntropy(nn.Module):
    def __init__(self, weight=None, ignore_index=255):
        super(MixVCDCrossEntropy, self).__init__()
        self.ignore_index = ignore_index
        if weight is None:
            self.ce = nn.CrossEntropyLoss(ignore_index=ignore_index)
        else:
    ...
    def vcd_loss(self, src, dst, y):
        valid_idx = (y != self.ignore_index)
        src, dst = src[valid_idx], dst[valid_idx]
        return self.kldiv(F.log_softmax(src.detach(), dim=-1), F.softmax(dst, dim=-1))
    ...
    def forward(self, out_dict: dict, y):
        ...
        l1 = self.ce(x_vox, y)  # 使用点云特征的预测结果
        l2 = self.ce(x_pix, y)  # 使用图像特征的预测结果
        x_vib = out_dict.get('vib_out')
        l3 = self.ce(x_vib, y)
        vcd = self.vcd_loss(x_vox, x_pix, y)
        vsd = self.vcd_loss(x_vox, x_vib, y)
        return {
            'predict_vox': l1,
            'predict_pix': l2,
            'predict_vib': l3,
